...
def contribution_whisker(input_percentages, levels, long_names,
    ...
    fig, axes = plt.subplots(nrows=1, ncols=1, figsize=(13.5, 82 * n_profile_vars/82 ))
    axes = [axes]

    for i, ax in enumerate(axes):

        var_to_plot = list(range(n_profile_vars))
        ...
        if not l_violins:
            ...
            # have time indices. Single time steps in the series will be copied to the multiple same time steps in the
            # frame. Manually this could be achieved by series.reindex_like(frame).
