...
# .sel(lat=slice(-20, -10), lon=slice(120, 134)) # west Australia
# .sel(lat=slice(-1, 5), lon=slice(-52, -44)) # Amazonas Delta
# .where(coast_mask & ((160 < rome['lon']) | (rome['lon'] < -90)) & (0 < rome['lat']), other=np.nan) # North Pacific
# map_to_plot = rhlow_p90.sum(dim='time').sel(lat=slice(-1, 5), lon=slice(-52, -44)) # Amazonas Delta
...
ax.coastlines()
map_to_plot.plot(ax=ax, cmap='plasma', vmin=0. , vmax=2.5)# cmap='BrBG')#, )
# ax.pcolormesh(map_to_plot)#, vmin=0. , vmax=2.5)# cmap='BrBG')#, )
# ax.collections[0].colorbar.set_label('Count ROME_p90 below 40% RH [1]')

plt.savefig(home+'/Desktop/map', bbox_inches='tight', transparent=True)
# plt.show()

# fig, ax = plt.subplots(figsize=(48, 3))
# # rome_southofindia.plot()
# rome_domain = rome.sel(lat=slice(-1, 5), lon=slice(-52, -44))
...
# plt.title(f'P90 avg. Amazonas-delta, p90_domainpixel={round(np.nanpercentile(rome_domain, q=90))}')
